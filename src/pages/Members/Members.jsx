...
export default function Members() {
  ...
  const MemberCard = ({ m }) => {
    ...
    const handleApprove = async () => {
      if (!isPending) return;
      try {
        await API.post("/api/mentor/dashboard/", {
    ...
    const handleReject = async () => {
      if (!isPending || !confirm("Reject this request?")) return;
      try {
        await API.post("/api/mentor/dashboard/", {
          request_id: m.request_id,
          action: "reject",
        ...
        <div className="member-content">
          ...
          <div className="member-info">
            <p className="member-line name-line">
              {m.full_name || "Unnamed"}
              {isPending && <span style={{ marginLeft: 8, color: "#f39c12", fontSize: "0.85rem" }}>(Pending)</span>}
            </p>
            <p className="member-line">
              {m.user_type === "mentor" ? "Mentor" : m.user_type === "guest" ? "Guest" : "Devotee"}:
              {m.member_id || "N/A"}
            ...
            {m.mobile && <p className="member-line">Mobile: {m.mobile} (Personal)</p>}
            {isPending && m.request_created_at && (
              <p className="member-line" style={{ color: "#777", fontStyle: "italic" }}>
                Requested: {formatDate(m.request_created_at)}
              </p>
            )}
            {isPending && (
              <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
                <button onClick={handleApprove} className="convert-btn" style={{ background: "#27ae60" }}>
                  Approve
                </button>
                <button onClick={handleReject} className="convert-btn" style={{ background: "#e74c3c" }}>
                  Reject
                </button>
              </div>
            )}
  ...
  const tabs = [
    ...
    { id: "devotee", label: `Devotees (${totalMentees})` },
    { id: "guest", label: `Pending (${pendingCount})` },
  ];
    ...
    <div className="profile-page">
      ...
      <div className="profile-content">
        <h3 style={{ marginBottom: 10 }}>
          ...
          {activeTab === "devotee" && "Your Devotees"}
